...
firebase = pyrebase.initialize_app(LoraRescueStorage)
db = firebase.database()
databaseEntries = db.get()
x = json.dumps(list(databaseEntries.val().items()))
df = pd.read_json(x)
entries1 = df.iloc[:, 0].tolist()
fig = 1

with open(save_destination+'Average Error.csv', mode='a') as clogs:
                clogswrite = csv.writer(clogs, dialect='excel', lineterminator='\n')
                clogswrite.writerow(["Date","Phone","Ave E","Max E","Min E","Diff A","Diff B","Diff C"])
...
for z1 in entries1: # "2021-10-30" "2021-11-06" "2021-11-07" "2021-11-13"
    ...
    # if len(z1) < 11 and z1 != "2021-5-20" and z1 != "2021-11-13":  # COMMENT THIS FOR 2ND RUN
        ...
        for z2 in entries2:
            ...
            print(z1)
            print(z2)
            rssiA, rssiB, rssiC, dtn, phoneA, latg, longg, latAct, longAct =  importDatabase(z1,z2)
            ...
            if z1 == "2021-10-30":
                ...
                firebase = pyrebase.initialize_app(LoraRescueStorage)
                db = firebase.database()
                n = db.child(z1).child(z2).child("Distance Calculation Constants").child("n").get().val()
            nA = n
            nB = n
            nC = n
            ...
            # CSV Writing
            diffAf = AfAve - comp_distanceAf
            diffBf = BfAve - comp_distanceBf
            diffCf = CfAve - comp_distanceCf
            centVactAve = sum(centVact)/len(centVact)
            with open(save_destination+'Average Error.csv', mode='a') as clogs:
                clogswrite = csv.writer(clogs, dialect='excel', lineterminator='\n')
                clogswrite.writerow([dtn,'0'+phoneA,str("{:.4f}".format(compVactAve[0])),str("{:.4f}".format(compVactMax[0])),str("{:.4f}".format(compVactMin[0])),diffAf,diffBf,diffCf,triImprovement,centVactAve])
