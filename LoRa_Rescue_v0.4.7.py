...
def importCSV(save_destination, startrow, endrow):
    ...
    return phone, dtn, rssiA, rssiB, rssiC
...
# Read rawData.csv to retrieve RSSI data, phone number, and timestamp
startrow = 60
endrow = 118
phoneA, dtn, rssiA, rssiB, rssiC = importCSV(save_destination, startrow, endrow)
...
# Save HTML Map File
m.save(save_destination + dtn + ' 0' + phoneA + ' K-Means.html')

# Screenshot HTML Map File to get PNG File
options = webdriver.ChromeOptions()
options.add_experimental_option('excludeSwitches', ['enable-logging'])
driver = webdriver.Chrome(browser_driver, options=options)
driver.get(save_destination + dtn + ' 0' + phoneA + ' K-Means.html')
time.sleep(5) # Delay to accomodate browser snapshotting, change accordingly
driver.save_screenshot(save_destination + dtn + ' 0' + phoneA + ' K-Means.png')
driver.close()
